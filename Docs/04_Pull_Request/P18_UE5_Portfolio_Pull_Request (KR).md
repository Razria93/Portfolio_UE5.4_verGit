...
본 PR은 Codex를 적극 활용하기 위한 `AI Workflow v1` 문서 구조와 `Prompt Library v1` 초안을 구성한 작업임.

이번 작업은 실제 UE C++ 기능 구현이 아니라, 자연어 요청을 `Work Brief -> Feature Work Planning -> Work Checklist Draft`로 변환하는 AI 기반 작업 흐름을 문서화하고 제한적으로 검증하는 것을 목표로 함.

### 이전 구조의 한계점

기존 문서 체계에서는 AI와 함께 작업할 때 다음 기준이 명확히 분리되어 있지 않았음.
...
- Work Brief / Planning / Checklist의 역할 경계
- 후속 Refactor 범위
```
...
4. D20 제한적 실사용 검증
- Parry 자연어 요청을 Work Brief / Feature Work Planning / Work Checklist Draft로 변환
...
`Docs/08_AI_Workflow`를 AI 기반 작업 운영 체계의 루트로 정리함.
...
Prompt 제작 / 유지보수 / 라우팅 기준을 `00_Prompt_Blueprint` 아래에 정리함.
...
-> Work Brief와 Work Planning 결과를 실행 관리용 Work Checklist로 변환
...
자연어 요청 이후의 Prompt 호출 흐름을 다음 계층으로 정리함.
...
Parry 구현 요청을 예시로 사용해 AI Workflow의 문서 변환 흐름을 검증함.
...
- Feature Work Planning을 Work Checklist Draft로 변환할 수 있는가
...
D20 문서 3종은 AI Workflow 제한적 실사용 검증 예시이며, 실제 Parry 구현 완료 문서가 아님.

---

### F. 리뷰 대응 반영

PR 리뷰에서 지적된 D20 입력 기준의 확정성 문제를 반영함.
...
실제 D20 Parry 구현은 별도 Branch에서 진행하도록 분리함.
...
```yaml
문서 출력 확인
-> D19 Work Checklist UTF-8 출력 확인

검색 검증
-> Branch 목표 / Prompt Library v1 / AI Workflow / D20 흐름 반영 여부 확인
...
D20 검증
-> D20 Work Brief / Feature Work Planning / Work Checklist Draft 문서 흐름 확인
-> D20 Work Checklist Draft에서 실제 구현 / Build / PIE / Editor / Asset 항목이 완료 처리되지 않았는지 확인
```
...
본 PR은 문서 / Prompt Library 구성 작업이므로 UE C++ 빌드는 수행하지 않음.
...
-> AI_Workflow_Refactor_Notes 기준으로 Prompt Flow / 문서 체계 / Work Checklist 갱신 규칙 재검토
-> 실제 D20 구현 중 Workflow 사용성 확인 후 필요한 부분만 반영
```
