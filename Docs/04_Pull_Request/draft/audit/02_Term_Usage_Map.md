...
P01~P18 draft 총검토에서 수집한 용어를 바탕으로, PR draft 전체에 적용할 통합 용어 기준과 문서별 사용 현황을 정리한다.

이 문서는 PR draft가 추가될 때마다 갱신하는 용어 기준 문서다. 확정된 기준은 본문 일괄 보완에 사용하고, 새 판단이 필요한 항목은 확정 전 별도 후보로 관리한다.

분류, 섹션별 적용, 통합 / 문서별 맵 구성 기준은 `01_Term_Usage_Classification_Guide.md`를 따른다.

## 운영 기준

- 이 문서는 PR draft 본문 보완에 사용할 실제 용어 목록과 문서별 사용 현황을 관리한다.
...
P01~P18 draft 전체에 공통 적용할 기본 용어 기준이다. 개별 문서에서 별도 override가 없으면 이 기준을 우선한다.
