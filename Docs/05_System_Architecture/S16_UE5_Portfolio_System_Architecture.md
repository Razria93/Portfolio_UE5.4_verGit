# 1차 Action Orchestration 리팩터링 시행착오와 구조적 결론

## 1. 제목

M05-S16: 1차 Action Orchestration 리팩터링 시행착오와 구조적 결론

---

## 2. 목적

본 문서는 1차 action orchestration refactor 과정에서 실제로 혼란이 컸던 지점과, 그 시행착오를 통해 정리된 구조적 결론을 기록하기 위한 문서임.

1차 `feature/action-orchestration` 브랜치의 어려움은 단순히 API 이름을 바꾸거나 action 실행 코드를 정리하는 데 있지 않았음.
...
기존 action execution Flow에서는 action component 내부에 query와 decision 성격의 처리가 많이 남아 있었음.
...
### Reaction Replace 모델과의 차이

Reaction 쪽에서는 이미 active reaction을 다른 reaction으로 교체하는 Flow가 `Replace`에 가까운 형태로 구현되어 있었음.
...
`ComboAttack`, `Equip`, `Unequip`, `Dodge`, 이후 추가될 `Guard`, `Parry`, `Counter`는 모두 같은 request Flow로 들어오더라도 runtime state에 따라 다른 action data key로 해석될 수 있음.
...
2. Action -> Reaction
-> hit reaction이 active action을 끊고 들어오는 Flow는 interrupt에 가까움

3. Action -> Reaction stop only
-> 어떤 경우에는 active reaction을 멈추기만 하고 후속 action을 시작하지 않을 수도 있음

4. Reaction -> Action
-> dodge처럼 active reaction을 cancel하고 action으로 진입하는 Flow가 있음
```
...
당시 구현은 local decision을 먼저 구하고, 그 decision을 policy가 필터링하는 형태에 가까웠음.
...
이 구조는 동작은 가능하지만, executor rule이 전체 body state나 active reaction 여부 같은 외부 상태를 알아야 하고, policy도 다시 local decision을 보고 허용/거부를 판단해야 함.

결과적으로 local과 policy의 책임이 겹침.

더 적절한 방향은 `policy`라는 모호한 중간 개념을 줄이고, 실행 환경을 `ExecutionSnapshot`으로 먼저 압축한 뒤 executor rule에게 전달하는 구조임.
...
`ExecutionSnapshot`은 body state, active action/reaction 유무, active context 유효성, 생존 여부처럼 executor rule이 전역 컴포넌트를 직접 조회하지 않도록 필요한 상태를 압축해야 함.
...
따라서 이 단계는 `Policy`보다 `InterventionQuery -> InterventionAssessment -> InterventionDirective` Pipeline으로 보는 것이 더 명확함.

```yaml
1. InterventionQuery
-> active와 incoming의 domain, context, requested intent, snapshot을 담음

2. InterventionAssessment
-> incoming want rule, active allow rule, priority, force 판단 결과를 담음

3. InterventionDirective
-> 무엇을 멈추고, 왜 멈추고, 멈춘 뒤 무엇을 할지 component가 소비할 수 있게 표현함
```

### Orchestration / Component 책임 경계
...
```yaml
1. FActionExecutionContext
2. FReactionExecutionContext
```
...
```yaml
1. ActionLocalDecision::Cancel
2. ReactionLocalDecision::Interrupt
...
당시 action/reaction orchestration은 자기 캐릭터의 실행 상태를 중심으로 판단함.
...
-> executor rule과 orchestration result를 통해 실행 결정을 구성함
...
당시 구현에서는 아직 `FActionResolvedPolicy`가 local decision 이후에 존재함.

따라서 당시 구조는 최종형이 아니라 중간 단계임.

다만 이번 브랜치에서 중요한 변화는 action component 내부에 있던 query와 decision Pipeline을 orchestration 쪽으로 끌어올렸다는 점임.
...
`ExecutionSnapshot`은 executor rule이 외부 컴포넌트를 직접 조회하지 않아도 되도록 실행 환경을 압축해야 함.
...
2. bIsAlive
3. bHasActiveAction
4. bHasActiveReaction
5. ActiveActionContext
6. ActiveReactionContext
```

### Intervention Query / Assessment / Directive

두 번째는 intervention arbitration을 `Query / Assessment / Directive`로 나누는 것임.

```yaml
1. FExecutionInterventionQuery
-> incoming과 active의 context, domain, intervention intent, snapshot을 담음

2. FExecutionInterventionAssessment
-> incoming wants, active allows, priority, force, window 판단 결과를 담음

3. FExecutionInterventionDirective
...
3. FActionLocalLevelQuery
4. FReactionLocalLevelQuery

5. FActionLocalLevelResult
6. FReactionLocalLevelResult
```
...
```yaml
1. EExecutionDomain
2. EExecutionInterventionIntent
3. EExecutionStopReason
4. EExecutionStopSource
5. EExecutionAfterStopAction
```
...
당시 구조는 아직 완성된 orchestration 모델이 아니었음.
...
3. Executor
-> 실행 기능을 집약하고, montage / notify / feedback / executor rule을 담당한다.
