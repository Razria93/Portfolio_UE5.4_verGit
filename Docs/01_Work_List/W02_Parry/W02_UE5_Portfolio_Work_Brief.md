...
- Parry Animation 입력 Asset과 후속 Architecture baseline은 구현 착수 전에 확인해야 한다
...
- `UCCombatResolutionComponent` 신규 구성 비용이 있다
- `CAction_Parry`와 기존 WindowKey 모델의 Window Open / Close 동기화가 필요하다
- `ECombatActionIntent` / `EActionType`에 Parry가 없어 입력 요청과 ActionDataKey 연결을 추가해야 한다
- Parry Action에서 Parry Reaction으로 interrupt하는 흐름이 기존 Montage lifecycle과 충돌할 수 있다
- Damage 무효화와 Feedback 실행 순서가 어긋날 수 있다
