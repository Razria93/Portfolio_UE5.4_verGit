...
- 최종 결과는 `ApplyDamageToTarget` 한 지점에서만 타깃에게 전달하며, 이 함수에서 엔진 `TakeDamage(...)`를 호출함(수신부 구현은 M2-04에서 처리).
...
- [x] 호출 경로는 구조화된 로그로만 검증함(`TakeDamage` 수신 구현은 M2-04, HP/UI는 M2-05에서 처리)
