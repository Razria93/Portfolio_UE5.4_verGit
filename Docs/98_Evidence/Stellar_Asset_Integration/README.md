...
| --- | --- | --- | --- | --- | --- |
| E01 | 작은 체구의 상체 눌림을 줄이고 새 캐릭터 세트 사용 | `a6e79e34` → `13299da5`; 세션 A:93,129,400 | `Content/01_Character/Retager/RTG_AutoGenerated.uasset`, StellarGirlCeleste, BP_CPlayer_Stellar | 당시 Alpha 0.7 시각 개선 사용자 확인; 현재 이주 에셋 존재; 최신 Player `62378b3f` | 현재 Retargeter 값과 전체 export 설정 미검증 |
...
| E06 | 환경 맵을 통합하고 충돌·조명 구성 | `c543cfa3` → `13299da5` | `Content/SciFi/SciFiMap.umap` | 도입 이력·파일 확인 / 사용자 작업 진술 | 구체적 설정·판단·성공 증거는 추가 조사 기록 참조 |
| E07 | 필요 에셋 보존과 경로 정리 | `a6e79e34`, `90ba6748`, `8d6905bf` → `13299da5` | `T_CELESTE_FlatNormal.uasset`, Content 새 경로 | 파일명 정리 이력 확인; 세션 C:27,175,182,196 | material graph 검사 결과는 당시 assistant 도구 기록, 현재 재검사 아님 |
