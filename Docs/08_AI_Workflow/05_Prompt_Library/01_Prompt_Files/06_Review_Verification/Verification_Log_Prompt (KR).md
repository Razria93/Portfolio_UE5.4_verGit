...
- PIE 검증은 단순 실행 여부보다 outcome / commit / state / reaction / feedback / packet 같은 결과값 기준으로 기록해줘.
- 디버깅 로그는 임시 위치 추적 로그와 최종 회귀 로그를 구분하고, 최종 문서에는 유지할 로그 기준만 남겨줘.
...
-> 미검증 항목이 구체적인가
-> PIE 검증 결과가 관찰 가능한 결과값 기준으로 기록되었는가
-> 임시 디버깅 로그와 최종 회귀 로그가 구분되었는가
```
