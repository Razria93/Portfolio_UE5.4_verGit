# Documentation Index Writing Prompt

## 1. 목적

문서군별 Index와 전체 Documentation Index를 작성하거나 보완할 때, 상위 라우터와 카테고리별 상세 목록의 역할을 분리해 정리한다.
...
-> 문서군 prefix / 파일명 / 폴더명이 변경되어 Index 갱신이 필요할 때
-> 문서군별 상세 목록과 상위 Index의 역할이 섞였는지 점검할 때
```
...
대상 문서군, 실제 파일 경로, 문서 ID prefix, 현재 폴더 구조, 관련 문서군을 입력하고 `복사용 Prompt`를 사용한다.

상위 Index와 문서군별 Index를 함께 작성할 경우, 상위 Index는 문서군 라우터로 두고 상세 문서 목록은 각 문서군별 Index에 둔다.
...
````text
다음 문서군의 Documentation Index를 작성하거나 보완해줘.
...
작성 / 보완 목적:
- [새 Index 작성 / 파일명 변경 반영 / 문서군 재분류 반영 / 구형 명칭 제거 등]
...
```yaml
검증 기준
-> 전체 Index가 문서군 라우터 역할만 하는가
-> 문서군별 Index에 상세 목록이 있는가
