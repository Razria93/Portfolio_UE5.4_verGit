...
- W03 v1에서는 `CombatResultPacket(Parry)` 수신자가 `ParryStack`을 누적하고, threshold 도달 시 `CombatResult` 기반 `Stagger` reaction request를 생성한다.
  - `Stagger` reaction이 accepted되면 stack을 reset하고, rejected되면 현재 stack을 유지해 다음 검증에서 원인을 추적한다.
