...
**M01-03: Movement / Jump 및 기본 Locomotion AnimBP 세팅**
...
- Player Movement(Idle/Walk/Run) 기본 로직 구현
...
- [x] `CPlayer`에 MoveForward / MoveRight 함수 추가
...
- [x] Idle / Walk / Run용 애니메이션 리소스 선정

- [x] `BP_CPlayer`용 AnimBlueprint 생성

- [x] Speed 기반 Idle/Walk/Run BlendSpace 생성

- [x] AnimBP에서 Locomotion State(Idle/Move) 구성

- [x] `CPlayer`에서 AnimBP에 전달할 Speed 변수 값 세팅

#### 3. Git / Issue

- [x] Day3용 GitHub Issue 생성 (제목: `M01-03: Movement / Jump 및 기본 Locomotion AnimBP 세팅`)
...
- **애니메이션 자산 마이그레이션 이슈 및 해결 (M01-B01: #7)**
...
- **향후 Locomotion 고도화 계획**
    기본 Locomotion 구현 이후, 8-Way 기반 방향 전환, 애니메이션 이동 거리 기반의 모션 보정, 발 동기화(Foot Sync) 등을 포함한 정교한 Locomotion 시스템으로 확장할 예정임
