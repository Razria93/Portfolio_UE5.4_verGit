...
- notify는 runtime window timing만 전달하고, policy는 data rule이 소유하는 구조로 정리되었다.
  
- montage 0 frame ~ last frame notify state 배치 문제는 policy cache를 notify에 직접 싣지 않는 구조로 위험 범위를 줄였다.
