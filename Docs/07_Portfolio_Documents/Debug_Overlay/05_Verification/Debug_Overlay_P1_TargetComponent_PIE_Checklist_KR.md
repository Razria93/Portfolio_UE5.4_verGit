...
| nearest radius | `1500.f` |
| nearest 기준 | camera 방향이 아니라 player pawn 위치 기준 거리 |
| nearest diagnostic | HUD `EnemySelect:` line과 Output Log로 확인 |
...
`EnemySelect:` line은 command 실행 결과를 확인하기 위한 진단 정보다. 성공 evidence 자체가 아니라, 선택 성공/실패 이유를 설명하는 보조 정보로만 사용한다.
...
| 명령 | 동작 | 성공 기대값 | 실패 기대값 |
| --- | --- | --- | --- |
| `DebugOverlaySelectNearestTarget` | player pawn 기준 nearest `ACEnemy`를 명시 선택 | `EnemySource: TargetComponent.Nearest`, `EnemyTarget: Selected=...`, `EnemySelect: NearestSelected \| Target=... \| Distance=... \| Radius=1500` | 기존 target clear 후 `EnemySource: None`, `EnemySelect: NearestFailed \| OutOfRange/NoEnemy/InvalidContext ...` |
| `DebugOverlayClearTarget` | explicit target clear | `EnemySource: None`, `EnemySelect` line 없음 | 해당 없음 |
...
| 2 | Enemy 근처에서 `DebugOverlaySelectNearestTarget` 실행 | `EnemySource: TargetComponent.Nearest`, `EnemyTarget: Selected=...`, `EnemySelect: NearestSelected \| Target=... \| Distance=... \| Radius=1500` | nearest radius `1500.f`, player pawn 위치, Enemy 거리 |
| 3 | Enemy가 너무 먼 상태에서 `DebugOverlaySelectNearestTarget` 실행 | `EnemySource: None`, `EnemySelect: NearestFailed \| OutOfRange \| Closest=... \| Radius=1500` | 실패 시 기존 target clear 여부, closest distance |
| 4 | Enemy가 없는 상태 또는 유효 Enemy 없음에서 `DebugOverlaySelectNearestTarget` 실행 | `EnemySource: None`, `EnemySelect: NearestFailed \| NoEnemy \| Radius=1500` | 테스트 map의 `ACEnemy` 존재 여부 |
| 5 | `DebugOverlayClearTarget` 실행 | `EnemySource: None`, `EnemySelect` line 없음 | clear 후 이전 target source 또는 diagnostic이 남아 있는지 |
...
- `EnemySelect` line은 선택 결과와 거리 제한을 설명하는 diagnostic이다.

### 7.2 Nearest 반경 밖 실패

```text
EnemySource: None
EnemySelect: NearestFailed | OutOfRange | Closest=2150 | Radius=1500
```

의미:

- world에 Enemy는 있지만 player pawn 기준 nearest radius `1500.f` 밖에 있다.
...
- `DebugOverlayClearTarget` 실행 후에는 `EnemySelect` line도 사라져야 한다.
...
| 시나리오 | 기대 로그 |
| --- | --- |
| nearest 성공 | `DebugOverlaySelectNearestTarget Result=Selected Target=... Distance=... Radius=1500` |
| 반경 밖 | `DebugOverlaySelectNearestTarget Result=OutOfRange Closest=... Radius=1500` |
| Enemy 없음 | `DebugOverlaySelectNearestTarget Result=NoEnemy Radius=1500` |
...
`TargetComponentMissing`은 component 자체가 없어서 HUD에 `EnemySelect:`로 표시하기 어렵다. 이 케이스는 Output Log 기준으로 확인한다.
...
- Enemy가 `1500.f` 밖에 있는데 `EnemySelect: NearestFailed | OutOfRange ...`가 보이지 않음
...
```text
EnemySource: None
EnemySelect: NearestFailed | OutOfRange | Closest=... | Radius=1500
```
...
- `DebugOverlayClearTarget` 이후 `EnemySelect` line이 남아 있음
...
- `EnemySelect`는 command 실행 결과 진단 정보이며, target 성공 evidence 본문으로 과장하지 않는다.
...
| Nearest 성공 diagnostic | `EnemySelect: NearestSelected \| Target=... \| Distance=... \| Radius=1500` 표시 |
| OutOfRange diagnostic | 기존 target clear 후 `EnemySource: None`, `EnemySelect: NearestFailed \| OutOfRange \| Closest=... \| Radius=1500` 표시 |
| NoEnemy diagnostic | `EnemySelect: NearestFailed \| NoEnemy \| Radius=1500` 표시 |
| Output Log | command 실행 결과 `Result=...` 로그 표시 |
| Clear | `DebugOverlayClearTarget` 후 `EnemySource: None`, `EnemySelect` line 없음 |
