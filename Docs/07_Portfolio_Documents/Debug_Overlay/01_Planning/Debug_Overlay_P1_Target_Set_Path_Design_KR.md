...
현재 `UCDebugOverlayTargetComponent`는 `ACPlayerController` 소유 component로 구현되어 있고, HUD Enemy source chain도 `TargetComponent -> RecentCombatTarget -> WorldScanFallback` 순서로 전환되어 있다. 다만 `SetDebugOverlayTarget()`을 호출하는 경로가 아직 없으므로 PIE에서 `EnemySource: TargetComponent`를 안정적으로 확인할 수 없다.
...
| HUD source chain | `TargetComponent -> RecentCombatTarget -> WorldScanFallback` |
| 누락 지점 | `SetDebugOverlayTarget()` 호출 경로 |

현재 상태에서 `EnemySource: TargetComponent`는 component target이 외부에서 세팅될 때만 표시된다. 따라서 다음 구현 단계에서는 `ACPlayerController`에서 debug-only helper를 통해 target component에 Enemy를 넣는 경로가 필요하다.
...
| nearest enemy scan | 구현 확실성이 높고 HUD fallback과 패턴이 유사함 | 다중 enemy에서 선택 근거가 약함 | trace 실패 fallback |
...
2. trace 실패 시 nearest `ACEnemy` fallback
3. clear command로 target 해제
...
- `WorldScanFallback`을 최후 fallback으로 유지할 수 있다.
...
| nearest fallback radius | `1500.f` 후보 | trace 실패 시 보조 |
...
clear는 target component의 명시 target만 해제한다. Store recent combat pair나 world scan fallback을 지우는 기능이 아니다.
...
| 선택 알고리즘 | camera trace 1순위, nearest fallback 2순위 | evidence 설명과 구현 안정성 균형 |
| clear command | 포함 | target source chain 하강 확인 가능 |
...
4. camera forward trace와 nearest fallback은 `ACEnemy`만 반환한다.
5. 명시 target clear 경로를 포함한다.
6. Shipping에서는 no-op 또는 컴파일 제외한다.
7. PIE에서 `EnemySource: TargetComponent`를 확인한다.
8. fallback chain이 기존처럼 유지되는지 확인한다.
...
- 선택 알고리즘은 camera trace 우선, nearest fallback 보조다.
- RecentCombatTarget은 자동 승격하지 않는다.
- clear command/helper는 target component만 비운다.
