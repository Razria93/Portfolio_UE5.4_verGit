...
이 문서는 `UCDebugOverlayFocusComponent`에 실제 target 값을 세팅하기 위한 debug-only 호출 경로를 확정한다.

현재 `UCDebugOverlayFocusComponent`는 `ACPlayerController` 소유 component로 구현되어 있다. P1 최종 source 정책은 `Debug_Overlay_P1_Focus_Selection_Decision_KR.md`를 우선하며, HUD는 명시 focus이 있을 때만 `FocusComponent.NearestFocus`를 표시한다.
...
| 항목 | 상태 |
| --- | --- |
| Focus component | `UCDebugOverlayFocusComponent` 구현 완료 |
| Owner | `ACPlayerController` |
| 저장 방식 | `TWeakObjectPtr<AActor>` |
| public API | `HasDebugOverlayFocus`, `GetDebugOverlayFocusActor`, `GetDebugOverlayFocusSummary`, `GetDebugOverlayFocusSource`, `SetDebugOverlayFocus`, `ClearDebugOverlayFocus` |
| HUD source policy | `FocusComponent.NearestFocus -> None` |
| 누락 지점 | `SetDebugOverlayFocus()` 호출 경로 |

현재 상태에서 `EnemyFocusMode: FocusComponent.NearestFocus`는 component target과 source type이 외부에서 세팅될 때만 표시된다. 따라서 다음 구현 단계에서는 `ACPlayerController`에서 debug-only helper를 통해 target component에 Enemy와 source type을 넣는 경로가 필요하다.
...
| camera forward trace | "보고 있는 Enemy 선택"이라는 evidence 설명이 명확함 | PIE에서 맞추기 어렵고 향후 FocusComponent 방향과 다름 | P1 제거 |
| nearest enemy scan | 구현 확실성이 높고 명시 command 기반 선택으로 설명 가능 | 다중 enemy에서 선택 근거가 약함 | P1 기본 command |
| mouse cursor hit result | 화면 클릭 기반 선택 가능 | 현재 mouse interaction 패턴이 없고 PIE capture와 충돌 가능 | P1 보류 |
| RecentCombatFocus 자동 승격 | 실제 전투 상대 기반 | "명시 선택 target"과 "최근 전투 상대" 의미가 섞임 | 보류 |
...
1. nearest command로 nearest `ACEnemy` 선택
2. 실패 또는 clear command로 target 해제

이 방식은 다음 이유로 적합하다.

- target 선택 책임이 `ACPlayerController` 내부 debug-only 경로에 머문다.
- `UCDebugOverlayFocusComponent` API를 변경하지 않아도 된다.
- HUD와 Store 구조를 변경하지 않아도 된다.
- line trace를 맞추지 않아도 focus 없음/선택 성공을 명확히 확인할 수 있다.
- focus 없음 상태를 `EnemyFocusMode: None`으로 명확히 보여줄 수 있다.
- RecentCombatFocus은 자동 승격하지 않아 의미 혼동을 줄인다.
...
clear는 target component의 명시 focus을 해제하고 Enemy panel을 `EnemyFocusMode: None` 상태로 만든다. Store recent combat pair나 world scan diagnostic 데이터를 지우는 기능은 아니다.
...
| 선택 알고리즘 | nearest command 단일화 | line trace 운용 난이도를 줄이고 명시 선택만 유지 |
| clear command | 포함 | 명시 focus 없음 상태 확인 가능 |
...
2. `UCDebugOverlayFocusComponent` API는 가능하면 변경하지 않는다.
3. HUD/Store 구조는 변경하지 않는다.
4. nearest command는 `ACEnemy`만 반환한다.
5. 명시 focus clear 경로를 포함한다.
6. Shipping에서는 no-op 또는 컴파일 제외한다.
7. PIE에서 `EnemyFocusMode: FocusComponent.NearestFocus`를 확인한다.
8. 실패/clear 후 `EnemyFocusMode: None`을 확인한다.
...
- 선택 알고리즘은 nearest command로 단일화한다.
- RecentCombatFocus은 자동 승격하지 않는다.
- clear command/helper는 target component를 비우고 `EnemyFocusMode: None` 상태로 만든다.
