...
P0.5 Enemy 패널은 focus component 기반이 아니다. 현재는 world scan fallback으로 단일 enemy를 표시한다.
...
Focus Component 기반 enemy selection은 P1에서 검토한다. Blackboard 기반 선택은 Target Component가 없거나 AI 내부 상태를 보조 검증해야 할 때의 보조 후보로 둔다.
...
| Enemy selection | world scan fallback이므로 Focus Component 기반 evidence가 아니다. |
...
| 후보 | 목적 |
| --- | --- |
| Focus Component 기반 enemy selection | gameplay/runtime truth인 현재 target 기준 enemy 표시 |
