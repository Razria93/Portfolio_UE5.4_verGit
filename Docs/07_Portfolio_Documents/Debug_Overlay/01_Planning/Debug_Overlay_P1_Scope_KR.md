...
| 목표 | 설명 |
| --- | --- |
| Enemy panel 신뢰도 강화 | `WorldScanFallback` 중심 claim에서 벗어나 실제 focus source 기반 표시로 전환한다. |
| Focus source 명시 | overlay에 enemy source가 `FocusComponent.NearestFocus`, `None` 중 무엇인지 명확히 표시한다. |
...
| 필수 항목 | 포함 내용 | 완료 기준 |
| --- | --- | --- |
| Focus Component 기반 Enemy Selection 설계 | focus source, 명시 선택/None 표시 정책 확정 | 설계 문서 작성 |
| Focus Component / Target Provider 구현 | 현재 target actor를 제공하는 최소 구조 구현 | HUD에서 target actor 조회 가능 |
| HUD EnemyFocusMode 전환 | `FocusComponent.NearestFocus` 명시 선택 우선, focus 없음은 `None` 표시 | overlay에 source와 선택 대상 표시 |
...
## 4. EnemyFocusMode Selection Policy

P1의 enemy source selection policy는 `Debug_Overlay_P1_Focus_Selection_Decision_KR.md`를 우선한다.
...
| --- | --- | --- |
| `FocusComponent.NearestFocus` | 사용자 명령으로 nearest enemy를 명시 선택한 target | 명시 command 기반 focus selection evidence |
| `None` | 명시 focus 없음 | Enemy panel target evidence 없음 |

`RecentCombatFocus`과 `WorldScanFallback`은 P1 기본 source chain에서 제외한다. 이후 diagnostic/source 검증 후보로 유지할 수 있지만, focus 없음 상태에서 Enemy panel을 자동으로 채우지 않는다.

범위 제한:

- Target Component는 debug overlay가 읽을 현재 target source를 제공하는 최소 구조로 제한한다.
- 완전한 lock-on 시스템, target cycling UI, focus selection gameplay는 P1 필수 범위가 아니다.
- `RecentCombatFocus`은 actor raw pointer 장기 보관을 피한다.
...
| Runtime LOD 실제 표시 | Enemy `Runtime LOD: N/A` 해소 | 안정적인 enemy focus source |
| AI 표시 보강 | `AI: NotCaptured` 감소 | Focus Component 이후 AI source 검토 |
| 다중 enemy selector 검토 | 다중 enemy 상황 표시 개선 | Focus source 정책 확정 |
...
| 기준 | 완료 판단 |
| --- | --- |
| `EnemyFocusMode: FocusComponent.NearestFocus` 표시 가능 | Focus Component target이 있을 때 HUD에 표시 |
| focus 없음 표시 | 명시 focus이 없을 때 `EnemyFocusMode: None` 표시 |
| 자동 fallback 제한 | RecentCombatFocus/WorldScanFallback이 focus 없음 상태를 자동으로 채우지 않음 |
...
P1 Scope 확정 이후 다음 작업은 `Focus Component 기반 Enemy Selection 설계`다.

권장 흐름:

```text
P1 Scope 확정
Focus Component 기반 Enemy Selection 설계
Focus Component / Target Provider 구현
HUD EnemyFocusMode 전환
EventLog category filter
P1 검증 체크리스트
P1 검증
...
- Focus Component 구현 전에는 enemy claim을 fallback 기준으로만 설명한다.
...
P1에서 반드시 닫을 것은 Focus Component 기반 Enemy Selection, HUD EnemyFocusMode 전환, EventLog category filter, P1 검증이다. Store subject 분리, Player/Enemy Recent/EventLog 분리, Runtime LOD/AI 보강은 보강 범위로 두고, 구현 부담이 커지면 P2로 넘긴다.
